...
--  Migration: 00001_initial_schema (unified, replaces all prior versions)
...
CREATE TABLE IF NOT EXISTS public.user_profiles (
  ...
  tier                TEXT        NOT NULL DEFAULT 'free'
                                  CHECK (tier IN ('free','lucid','oracle')),
...
-- ============================================================
--  2. USER SETTINGS  (granular columns for fast queries)
-- ============================================================
...
-- ============================================================
--  3. USER PUSH TOKENS  (Expo push tokens for native apps)
-- ============================================================
...
-- ============================================================
--  4. WEB PUSH SUBSCRIPTIONS  (PWA Web Push / VAPID)
-- ============================================================
...
CREATE INDEX push_subscriptions_user_active ON public.push_subscriptions(user_id) WHERE is_active = TRUE;
...
CREATE POLICY "Users read own notification log" ON public.notification_log FOR SELECT USING (auth.uid() = user_id);


-- ============================================================
--  6. DREAMS  (core content table)
-- ============================================================
CREATE TABLE IF NOT EXISTS public.dreams (
  ...
  analysis          JSONB,
  -- analysis shape:
  -- { emotionalTone, keySymbols, narrativeArc, jungianArchetypes, shadowElements }
...
CREATE INDEX dreams_user_favorite   ON public.dreams(user_id, is_favorite) WHERE is_favorite = TRUE;
CREATE INDEX dreams_user_style      ON public.dreams(user_id, style_id);
CREATE INDEX dreams_user_mood       ON public.dreams(user_id, mood_id);
...
CREATE INDEX dreams_search_vector   ON public.dreams USING GIN(search_vector);
...
-- ============================================================
--  7. DREAM SYMBOLS  (encyclopedia — admin-seeded)
-- ============================================================
CREATE TABLE IF NOT EXISTS public.dream_symbols (
  ...
  category         TEXT NOT NULL
                   CHECK (category IN ('nature','architecture','creatures','celestial','body','objects','figures','elements')),
...
-- ============================================================
--  8. DREAM SYMBOL VIEWS  (personalisation tracking)
-- ============================================================
...
CREATE POLICY "Users manage own symbol views" ON public.dream_symbol_views FOR ALL USING (auth.uid() = user_id);
...
-- ============================================================
--  9. STORAGE BUCKET — dream-audio  (voice recordings, private)
-- ============================================================
...
-- ============================================================
--  10. STORAGE BUCKET — dream-thumbnails  (AI images, public read)
-- ============================================================
INSERT INTO storage.buckets(id, name, public, file_size_limit, allowed_mime_types)
...
-- Streak calculation (server-side, timezone-safe)
...
-- ============================================================
--  13. SYMBOL SEED DATA  (matches constants/symbols.ts)
-- ============================================================
INSERT INTO public.dream_symbols
  (id, symbol, emoji, category, short_meaning, full_meaning, jungian_context,
   related_symbols, common_contexts, affirmation)
VALUES
('forest','Forest','🌲','nature',
 'The unconscious mind; the unknown self',
...
('ocean','Ocean','🌊','nature',
 'The collective unconscious; emotional depth',
...
('house','House','🏠','architecture',
 'The self; the structure of the psyche',
...
('door','Door','🚪','architecture',
 'Threshold; transition and opportunity',
...
('snake','Snake','🐍','creatures',
 'Transformation; hidden wisdom or primal energy',
...
('bird','Bird','🕊️','creatures',
 'Freedom; spirit; transcendence',
...
('moon','Moon','🌙','celestial',
 'The unconscious; cycles; the feminine principle',
...
('sun','Sun','☀️','celestial',
 'Consciousness; vitality; the self in fullness',
...
('water','Water','💧','elements',
 'The unconscious; emotion; flow and change',
...
('mirror','Mirror','🪞','objects',
 'Self-reflection; the shadow; alternate selves',
...
('key','Key','🗝️','objects',
 'Access; solution; hidden knowledge unlocked',
...
('shadow-figure','Shadow Figure','👤','figures',
 'The shadow; the unintegrated self',
...
('wise-elder','Wise Elder','🧙','figures',
 'Inner wisdom; the Self; guidance from depth',
